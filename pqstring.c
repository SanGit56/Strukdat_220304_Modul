...
void pqueue_push(PriorityQueue *pqueue, char *teks, int urutan);
...
void pqueue_push(PriorityQueue *pqueue, char *teks, int urutan)
{
    ...
    if (urutan < pqueue->_top->prio) {
        newNode->next = pqueue->_top;
        pqueue->_top = newNode;
    }
    else {
        while ( temp->next != NULL && 
                temp->next->prio < urutan)
            temp = temp->next;
...
int main(int argc, char const *argv[])
{
    ...
    pqueue_push(&myPque, "Nasabah Prioritas 2", 2);
    pqueue_push(&myPque, "Nasabah Prioritas 3", 3);
    pqueue_push(&myPque, "Nasabah Prioritas 1", 1);
